...
print OUTFILE "Serial #\tLast name\tFirst name\tMiddle name\tName Suffix\tSex\tSpecialty\tCity\tLast rated\t# of ratings\tOverall Quality\tURL to detailed ratings\n";
...
for ($i = 'A'; $i ne 'AA'; $i++) {
	...
	for ($j = 0; $j < scalar(@lines) && !$done; $j++) {
		...
		# Changed to improve last/first/middle name handling
		if ($lines[$j] =~ m/HREF="\/(doctor-ratings[^" ]*).*Click to see ratings.*\>([^<]+)\</) {
		    # We DO care about this next section!
		    # the variables we will fill in.
		    my ($lastName, $firstName, $middleName, $nameAppendix, $sex, $specialty, $city, $lastRated, $numRatings, $overallQuality, $url);
		    $url = "http://www.ratemds.com/$1";

		    #Handle names of form last, first middle.
		    #Everything before the comma is last (or, if no comma, all is last), then up to first space
		    # after comma is first, rest is middle.
		    my $nameTmp = $2;
		    $nameAppendix = "";
		    # Check for a name appendix like jr, sr, iii and remove it if found.
		    # A name appendix is assumed to be everything between two commas.
		    if ($nameTmp =~ m/,\s?([^,]+),/) {
			$nameAppendix = $1;
			$nameTmp =~ s/,\s?[^,]+,/,/;
		    }
		    ...
		    print OUTFILE tabSeparate("$stateFolder-$totalCnt", $lastName, $firstName, $middleName, $nameAppendix, $sex, $specialty, $city, $lastRated, $numRatings, $overallQuality, $url);
		    $totalCnt++;
		} 
	    }
	}   
