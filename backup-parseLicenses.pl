...
our $doAll = 1; # do everything.
our $doBoardCert = 1;
our $doOtherActions = 1;
our $doAcademicAppoint = 1;
our $doOtherHospitals = 1;
our $doPaidClaims = 1;
our $doPostGrad = 1;
...
if ($doAll) {
open(PRACTICEOFFICES, ">$stateFolder/data/PracticeOffices.txt") or die "Could not open output file $!";
open(NONENGLISHOFFICES, ">$stateFolder/data/NonEnglishOffices.txt") or die "Could not open output file $!";
open(NONENGLISHPRACTITIONER, ">$stateFolder/data/NonEnglishPractitioner.txt") or die "Could not open output file $!";
open(VAHOSPITALS, ">$stateFolder/data/VAHospitals.txt") or die "Could not open output file $!";
}
if ($doOtherHospitals) {
open(OTHERHOSPITALS, ">$stateFolder/data/OtherHospitals.txt") or die "Could not open output file $!";
}
if ($doAll) {
open(GRADSCHOOL, ">$stateFolder/data/GradSchool.txt") or die "Could not open output file $!";
}
if ($doPostGrad) {
open(POSTGRADSCHOOL, ">$stateFolder/data/PostGradSchool.txt") or die "Could not open output file $!";
}
if ($doAll) {
open(CONTINUINGED, ">$stateFolder/data/ContinuingEd.txt") or die "Could not open output file $!";
}
if ($doBoardCert) {
open(BOARDCERT, ">$stateFolder/data/BoardCertification.txt") or die "Could not open output file $!";
}
if ($doAll) {
open(PRACTICEAREAS, ">$stateFolder/data/PracticeAreas.txt") or die "Could not open output file $!";
open(INSURANCEPLANS, ">$stateFolder/data/InsurancePlans.txt") or die "Could not open output file $!";
open(HONORS, ">$stateFolder/data/HonorsAndAwards.txt") or die "Could not open output file $!";
}
if ($doAcademicAppoint) {
open(ACADEMIA, ">$stateFolder/data/AcademicAppointments.txt") or die "Could not open output file $!";
}
if ($doAll) {
open(PUBLICATIONS, ">$stateFolder/data/Publications.txt") or die "Could not open output file $!";
open(FELONY, ">$stateFolder/data/Felonies.txt") or die "Could not open output file $!";
}
if ($doOtherActions) {
open(ACTIONSBYOTHERS, ">$stateFolder/data/ActionsByOthers.txt") or die "Could not open output file $!";
}
if ($doAll) {
open(VAORDERSNOTICES, ">$stateFolder/data/VAOrdersAndNotices.txt") or die "Could not open output file $!";
}
if ($doPaidClaims) {
open(PAIDCLAIMS, ">$stateFolder/data/PaidClaims.txt") or die "Could not open output file $!";
}
open(DOCTORINFO, ">$stateFolder/data/DoctorInfo.txt") or die "Could not open output file $!";
...
sub parsePracticeOffices{ 
	...
	if ($lines[$i] =~ m/Practice Address/) {
	    ...
	    # Try to match the second line of address first in the same line as the street, then on the next line.
	    if ($lines[$i] =~ m/([^>]+), (\w\w) (\d\d\d\d\d)(\d\d\d\d)?/) {
		push(@city, $1);
		push(@state, $2);
		push(@zip, $3);
		if ($lines[$i] =~ m/\d\d\d\d\d(\d\d\d\d)/) {
		    push(@subzip, $1);
		} else {
		    push(@subzip, "");
		}
		...
		if ($lines[$i] =~ m/([^>]+), (\w\w) (\d\d\d\d\d)(\d\d\d\d)?/) {
		    ...
		    # We duplicate this code one more time to give a last attempt at finding that all-important zipcode.
		    $i++;
		    if ($lines[$i] =~ m/([^>]+), (\w\w) (\d\d\d\d\d)(\d\d\d\d)?/) {
			push(@city, $1);
			push(@state, $2);
